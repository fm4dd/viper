...
int main(int argc, char *argv[]) {
   ...
   printf("Wiltered Fire - www.wilter.com/wf, incl. bugfixes by David C. Rankin\n\n");
	...
	if (!file){printf("Error: Password filename required!\n"); exit(-1);}
	else if (!user){printf("Error: Username required!\n"); exit(-1);}
	...
	else { printf("Found: Charset %d in %s\n", chr, CHARSET_FILE); }
...
the_res(struct crack_input erg, char * endpass, struct tm start)
{
